...
for i in `seq 2 $nphenos`; do
  ...
  parallel_mlm="$parallel_mlm \n $TASSEL5 -fork1 -h $dummy_genos -fork2 -t $targetdir/4q_blups.$name.permuted.txt -fork3 -k $kinship -combine4 -input1 -input2 -intersect -combine5 -input3 -input4 \
      -mlm -mlmMaxP 1 -mlmCompressionLevel None -export $outdir/4q_$name -runfork1 -runfork2 -runfork3; mv $outdir/*${statsfile}.txt $outdir/4r_${name}_stats.txt"    # No compression because causes TASSEL to hang on a lot of the permutations
