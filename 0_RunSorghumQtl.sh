...
chroms=`tail -n +2 1b_genos_filtered.hmp.txt | cut -f 3 | sort | uniq | grep "^Chr" | sed s/Chr// `   # Copied from above step
commands=""
inputs=""
runs=""
for chr in $chroms; do
...
# # # Check genetic map in R/QTL just to be sure - TODO - didn't finish this one, just did enough to confirm results
...
# # I don't have the next step in the pipeline yet, but basically it gives a 3-lod interval from s1220 to s1222 (so, the target SNP and the two to either side)
